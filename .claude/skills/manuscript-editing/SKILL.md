...
- **Punctuation** — Dialogue punctuation, em-dash style (dialogue interruptions only for all pen names), CMOS/Oxford comma consistency
...
**Why alternate models (do not skip this):** A model that both writes a fix and then checks its own work grades itself too generously — it rationalizes its own choices as correct and reports back that the pass went great. Handing IDENTIFY and VERIFY to a *different* model than the one that did the FIX is what actually catches missed and newly-introduced issues. Always alternate where the session allows it, and never let the same model both write a replacement and sign off on it.

- **Sonnet for identification and verification**: Fast, thorough analytical reading. Catches issues systematically — including ones Opus introduced or waved through.
...
- **Re-scans use the opposite model** from the first scan (see "One Bounded Re-Scan" above).

When spawning Task subagents — keep this BOUNDED so the spawning itself doesn't stall the run:
- Spawn subagents **per pass, batched by chapters — not one subagent per chapter.** Give a single IDENTIFY subagent a batch of 4–5 chapters (see Parallelization Strategy).
- IDENTIFY subagents: include the manuscript chapter batch + the relevant checklist from references/
...
- **Hard cap per pass:** one IDENTIFY round, one FIX round, one VERIFY round, plus at most one bounded re-scan. Do not chain additional rounds beyond that.
- **If subagents are unavailable in the session, run the phases inline using the same model alternation.** Do not block or wait on subagents — falling back to inline is always preferable to hanging.
...
### 3. Minimum Issue Thresholds (One Bounded Re-Scan)

These are MINIMUM expected findings. Falling below these numbers is a signal you may have skimmed — so run **exactly ONE** additional re-scan, then STOP.

**Re-scan cap (this prevents the pipeline from hanging):** Each pass gets **at most one** re-scan. Run that re-scan with a DIFFERENT model than the one that produced the first scan — if Sonnet ran IDENTIFY, run the re-scan on Opus; if Opus did the work, re-scan on Sonnet. A second, independent model reliably catches what the first one rationalized as "good enough," which a self-review never will. After that single re-scan the count you have is FINAL. Do NOT re-scan a third time, and do NOT loop trying to reach the threshold number. A documented count below the minimum is an acceptable, terminal result — report it and move on.

| Pass | Minimum Issues per 10k Words | The single re-scan (only if below threshold) |
|------|-----------------------------|--------------------|
| **Proofread** | 5 issues | Re-scan every chapter ONCE, running each checklist category (grammar, punctuation, homophones, consistency) as a separate sweep within that one re-scan. |
| **Line Edit** | 10 issues | Re-scan chapter by chapter ONCE, running each checklist section as a separate sweep: AI artifacts, show-vs-tell, crutch phrases, dialogue, prose mechanics. |
| **Continuity Audit** | 3 issues per 10k words | Re-build reference tables ONCE with more detail. |
...
**After the single re-scan, if you still find fewer issues than the threshold:** This is a valid stopping point — do not keep re-scanning. Report the actual count, list EVERY specific check you ran on EVERY chapter, and explain what you looked for in each. The author should see proof of thoroughness, not a blanket "it's clean" declaration — but proof of thoroughness is a finished report, not an endless re-scan.
...
For each chapter, run the FULL checklist for that pass. Log issues per chapter. If a chapter comes back with zero issues, re-read it ONCE — ideally with a different model than the first read, since a zero-issue chapter in a proofread of fiction is rare. If it still reads clean after that single re-read, log it as zero and move on. Do NOT re-read the same chapter again and again chasing an issue.
