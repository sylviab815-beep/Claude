...
## Output Format Selection — Ask This First

**Before anything else — before questions, before generation — ask the user:**
...
Store their choice as `OUTPUT_FORMAT` and apply it consistently to **all** output sections for the entire plugin run.

- **If A (Markdown):** Skip all file-creation steps. Output everything as clean, well-structured markdown directly in chat — use headers (`##`, `###`), horizontal rules (`---`), bullet points, and bold labels. Do not create any `.docx` or `.pdf` files.
- **If B (Word Document):** Proceed with the existing `.docx` file creation workflow as normal.
- **If C (PDF):** Proceed with file creation, producing `.pdf` output instead of `.docx` where applicable.

Ask this question **once only**, at the very start. Do not ask it again mid-workflow.
